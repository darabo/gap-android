...
This document specifies the gossip-based synchronization feature for Gap Mesh, inspired by Plumtree. It ensures eventual consistency of public packets (ANNOUNCE and broadcast MESSAGE) across nodes via periodic sync requests containing a compact Golomb‑Coded Set (GCS) of recently seen packets.

## Overview

- Each node maintains a rolling set of public Gap Mesh packets it has seen recently:
...
Implementation: `com.Gap Mesh.android.sync.PacketIdUtil`.

## GCS Filter (On-demand)

Implementation: `com.Gap Mesh.android.sync.GCSFilter`.
...
- Header: normal Gap Mesh header with TTL indicating “local-only” semantics. Implementations SHOULD set TTL=0 to prevent any relay; neighbors still receive the packet over the direct link-layer. For periodic sync, recipient is broadcast; for per-peer initial sync, recipient is the specific peer.
...
Encode/Decode implementation: `com.Gap Mesh.android.model.RequestSyncPacket`.
