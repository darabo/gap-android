...
class NostrTransport(
    ...
    companion object {
        ...
        private const val READ_ACK_INTERVAL = com.gap.android.util.AppConstants.Nostr.READ_ACK_INTERVAL_MS // ~3 per second (0.35s interval like iOS)
        ...
        transportScope.launch {
            try {
                ...
                val recipientPeerIDForEmbed = try {
                    com.gap.android.favorites.FavoritesPersistenceService.shared
    ...
    fun sendPrivateMessageGeohash(
        ...
        val geohash = sourceGeohash ?: run {
            val selected = try {
                com.gap.android.geohash.LocationChannelManager.getInstance(context).selectedChannel.value
            } catch (_: Exception) { null }
            if (selected !is com.gap.android.geohash.ChannelID.Location) {
    ...
    private fun resolveNostrPublicKey(peerID: String): String? {
        try {
            ...
            com.gap.android.favorites.FavoritesPersistenceService.shared.findNostrPubkeyForPeerID(peerID)?.let { return it }

            // 2) Legacy path: resolve by noise public key association
            val noiseKey = hexStringToByteArray(peerID)
            val favoriteStatus = com.gap.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
            ...
            if (peerID.length == 16) {
                val fallbackStatus = com.gap.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)
