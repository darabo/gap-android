...
class BluetoothMeshService(private val context: Context) {
    ...
    /**
     * Check if a peer is reachable (direct or mesh)
     */
    ...
    fun sendMessage(content: String, mentions: List<String> = emptyList(), channel: String? = null) {
        ...
        // Prepend channel if provided, adhering to text-based routing convention (#channel message)
        val finalContent = if (!channel.isNullOrEmpty() && !content.trim().startsWith("#")) {
            val cleanChannel = channel.removePrefix("#")
            "#$cleanChannel $content"
        } else {
            content
        }
        ...
        serviceScope.launch {
            val packet = BitchatPacket(
                ...
                timestamp = System.currentTimeMillis().toULong(),
                payload = finalContent.toByteArray(Charsets.UTF_8),
                signature = null,
                ttl = MAX_TTL
            )
