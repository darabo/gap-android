...
class BluetoothConnectionManager(
    ...
    init {
        ...
        try {
            val dbg = com.gap.android.ui.debug.DebugSettingsManager.getInstance()
            // Role enable/disable
    ...
    override fun onPowerModeChanged(newMode: PowerManager.PowerMode) {
        ...
        connectionScope.launch {
            ...
            val serverEnabled = try { com.gap.android.ui.debug.DebugSettingsManager.getInstance().gattServerEnabled.value } catch (_: Exception) { true }
            if (serverEnabled) {
            ...
            if (wasUsingDutyCycle != nowUsingDutyCycle) {
                ...
                val clientEnabled = try { com.gap.android.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value } catch (_: Exception) { true }
                if (clientEnabled) {
            ...
            try {
                val maxServer = com.gap.android.ui.debug.DebugSettingsManager.getInstance().maxServerConnections.value
