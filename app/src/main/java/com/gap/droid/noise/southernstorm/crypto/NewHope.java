...
public class NewHope {
	 ...
	 * 
	 * @param send       Buffer to place the public key for Alice in, to be sent to
	 *                   Bob.
	 * @param sendOffset Offset of the first byte in the send buffer to populate.
	 * 
	 *                   The send buffer must have space for at least
	 *                   NewHope.SENDABYTES bytes
	 ...
	 * @param sharedkeyOffset Offset of the first byte in the sharedkey buffer to
	 *                        populate.
	 * @param send            Buffer to place the public key for Bob in to be sent
	 *                        to Alice.
	 * @param sendOffset      Offset of the first byte in the send buffer to
	 *                        populate.
	 * @param received        Buffer containing the public key value received from
	 *                        Alice.
	 * @param receivedOffset  Offset of the first byte of the value received from
	 *                        Alice.
	 * 
	 *                        The sharedkey buffer must have space for at least
	 *                        NewHope.SHAREDBYTES
	 *                        bytes starting at sharedkeyOffset.
	 * 
	 *                        The send buffer must have space for at least
	 *                        NewHope.SENDBBYTES bytes
	 *                        starting at sendOffset.
	 * 
	 *                        The received buffer must have space for at least
	 *                        NewHope.SENDABYTES
	 ...
	 * 
	 *               This function may be overridden in subclasses to provide a
	 *               better
	 *               random number generator or to provide static data for test
	 *               vectors.
	 */
	...
	private static void encode_b(byte[] r, int roffset, Poly b, Poly c) {
		...
		for (i = 0; i < PARAM_N / 4; i++)
			r[POLY_BYTES + roffset + i] = (byte) (c.coeffs[4 * i] | (c.coeffs[4 * i + 1] << 2)
	...
	private class Poly {
		...
		public void frombytes(byte[] a, int offset) {
			...
			for (i = 0; i < PARAM_N / 4; i++) {
				...
				coeffs[4 * i + 1] = (char) (((a[offset + 7 * i + 1] & 0xc0) >> 6)
						| ((a[offset + 7 * i + 2] & 0xff) << 2) | ((a[offset + 7 * i + 3] & 0x0f) << 10));
				coeffs[4 * i + 2] = (char) (((a[offset + 7 * i + 3] & 0xf0) >> 4)
						| ((a[offset + 7 * i + 4] & 0xff) << 4) | ((a[offset + 7 * i + 5] & 0x03) << 12));
				coeffs[4 * i
						+ 3] = (char) (((a[offset + 7 * i + 5] & 0xfc) >> 2) | ((a[offset + 7 * i + 6] & 0xff) << 6));
			}
		}
		...
		public void getnoise(byte[] seed, byte nonce) {
			...
			try {
				...
				for (i = 0; i < PARAM_N; i++) {
					 ...
					 * 
					 * t = (buf[4*i] & 0xff) | (((buf[4*i+1]) & 0xff) << 8) | (((buf[4*i+2]) & 0xff)
					 * << 16) | (((buf[4*i+3]) & 0xff) << 24);
					 * d = 0;
					 * for(j=0;j<8;j++)
					 * d += (t >>> j) & 0x01010101;
					 * a = ((d >>> 8) & 0xff) + (d & 0xff);
