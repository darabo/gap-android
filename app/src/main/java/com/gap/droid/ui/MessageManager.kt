...
class MessageManager(private val state: ChatState) {
    ...
    private val MESSAGE_DEDUP_TIMEOUT = com.gapmesh.droid.util.AppConstants.UI.MESSAGE_DEDUP_TIMEOUT_MS // 30 seconds
    private val SYSTEM_EVENT_DEDUP_TIMEOUT = com.gapmesh.droid.util.AppConstants.UI.SYSTEM_EVENT_DEDUP_TIMEOUT_MS // 5 seconds
    ...
    fun addMessage(message: BitchatMessage) {
        ...
        try { com.gapmesh.droid.services.AppStateStore.addPublicMessage(message) } catch (_: Exception) { }
    }
    ...
    fun addChannelMessage(channel: String, message: BitchatMessage) {
        ...
        try { com.gapmesh.droid.services.AppStateStore.addChannelMessage(channel, message) } catch (_: Exception) { }
    ...
    fun addPrivateMessage(peerID: String, message: BitchatMessage) {
        ...
        // Reflect into process-wide store
        try { com.gapmesh.droid.services.AppStateStore.addPrivateMessage(peerID, message) } catch (_: Exception) { }
    ...
    fun updateMessageDeliveryStatus(messageID: String, status: DeliveryStatus) {
        ...
        if (updated) {
            ...
            try { com.gapmesh.droid.services.AppStateStore.updatePrivateMessageStatus(messageID, status) } catch (_: Exception) { }
        }
